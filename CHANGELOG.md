...
- Advances the App SDK and local Add-ons workflow with reviewed multi-package composition, explicit host-mediated actions, bounded remote queries and attachments, sanitized provenance, transactional recovery, and catalog selection.
...
- Adds a Local Add-ons control panel in Apps & Features that reports the loaded custom build, preflights user-selected ZIPs, hands accepted packages to the checkout, and gives the exact Chrome Load unpacked or Reload step. The checked-in local manager remains responsible for authoritative validation and rebuilding.
