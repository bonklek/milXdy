...
published visual bundle must be VPL. This is the project contribution policy,
not a restriction on local packages.

An upstream submission includes:

- exact source and proposed exports with SHA-256 values;
- creator/source context and the explicit VPL evidence covering the file;
- the complete VPL text and required notices;
...
4. **License reviewer** — verifies exact VPL scope, notices, dependencies, and
   absence of additional restrictions.
...
local entries it verifies structure and safety while reporting license metadata
as informational. For upstream entries it enforces the project's VPL gate.
...
Deprecation or removal retains enough history for packages to migrate.
