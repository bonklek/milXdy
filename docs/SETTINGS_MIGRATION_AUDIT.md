...
| Composer Tools enablement | `local:milxdy.composerTools.enabled` | Main tab / Apps & Features card | Composer Tools feature | Apps & Features card, with popup mirror during migration | First-run, profile pack | Local-only post composer helper. Keep as feature enablement, not a global setting. | Yes |
...
2. Generate Apps & Features controls first for non-app feature modules: RemiStats, pokes, Tweet PNG, Composer Tools, wiki link highlighting, Root Visual feature toggles, and diagnostics disclosures.
