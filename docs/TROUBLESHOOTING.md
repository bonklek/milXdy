...
- **Open in LLM update handoff opens but does not paste**: the update steps are copied before milXdy opens the selected assistant target. Some sites and desktop apps block automatic paste or prefilled prompts, so paste the clipboard content manually into the new chat.
...
- **RemiNet Chat does not appear**: open **Apps & Features**, enable **RemiNet Chat**, and then refresh the X/Twitter tab. On X Messages, click the **RemiliaNET Chat** entry to open the larger RemiNet surface. The app is off by default.
...
- **Music cover art is missing**: reload the extension itself, refresh X/Twitter, and confirm the indexed folder still has read permission. Music rereads embedded artwork from the original local file when its parser version or file identity changes; rescan the folder if the file was replaced or moved.
- **Music enrichment finds no ISRCs**: confirm track metadata is readable and try MusicBrainz enrichment. Some local files or unofficial releases may not have reliable ISRC matches; you can also edit an ISRC manually.
