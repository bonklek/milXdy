...
- **Post-Factory cannot be selected in the Add-ons Catalog**: its record is intentionally **Under review** until the exact source and trusted catalog hash are committed together. Do not bypass the disabled catalog control; use only a separately reviewed package composition supplied through the documented local-package workflow.
- **Post-Factory quick replies or Factory controls are missing**: confirm the current custom build includes Post-Factory, enable it in **Apps & Features**, reload the same extension card, and refresh X tabs. Do not load a second unpacked folder.
