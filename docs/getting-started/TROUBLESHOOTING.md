...
- **Pets Maker was installed but is missing from the rail**: catalog installation deliberately leaves Pets Maker disabled. Open **Apps & Features**, enable **Pets Maker**, then pin or open it. Installing it never selects a file or generates a bundle.
- **Composer Kit cannot be selected in the Add-ons Catalog**: its record is intentionally **Under review** until the exact source and trusted catalog hash are committed together. Do not bypass the disabled catalog control; use only a separately reviewed package composition supplied through the documented local-package workflow.
- **Composer Kit quick replies or Factory controls are missing**: confirm the current custom build includes Composer Kit, enable it in **Apps & Features**, reload the same extension card, and refresh X tabs. Do not load a second unpacked folder.
- **Remibooru search returns no results**: simplify the tag query, clear selected facets, confirm `https://remibooru.com` is reachable, and try the next/previous controls only after a page has loaded. The picker does not fall back to scraping X or arbitrary image sites.
- **Publish to Remibooru fails**: keep the panel open and review its specific authentication, contributor-access, duplicate, format, size, tag, rate-limit, moderation, or network message. Sign in on Remibooru when requested, or use **Open native uploader** for CAPTCHA/anti-abuse and unsupported upstream states.
- **Share to RemiNet did not send the PNG**: staging is deliberately separate from sending. Confirm RemiNet Chat opened with the pending image, review the message, then press Chat's native Send control.
- **Miladychan text post failed**: the Portal retains the failed text locally while the panel remains available. Check the visible board/thread destination and use the native-site button for CAPTCHA, media, session-bound, unsupported-board, or full-board behavior.
