...
import { findXStatusHrefInText, normalizeXStatusUrl } from "./urlPolicy";
...
export function extractReadablePost(tweet: HTMLElement, settings: PostReadingSettings): ReadablePost | null {
  ...
  const quoteText = quoteElement ? cleanText(extractOwnTweetText(quoteElement, null, settings.includeHyperlinks)) : "";
...
export function formatReadablePost(post: ReadablePost, settings: PostReadingSettings): string {
  ...
  if (settings.includeQuotes && post.quote?.text) {
...
function extractQuoteUrl(quoteElement: HTMLElement, mainTweet: HTMLElement): string | null {
  ...
  if (!href) return null;
  try {
    const normalizedUrl = normalizeXStatusUrl(href, window.location.origin);
...
function findStatusHrefInText(value: string): string | null {
  return findXStatusHrefInText(value, window.location.origin);
}
...
function extractTweetTextNodeText(node: HTMLElement, includeHyperlinks: boolean): string {
  ...
  return isNonReadableTweetTextArtifact(text) ? "" : text;
}
...
export function isNonReadableTweetTextArtifact(value: string): boolean {
  const text = cleanText(value);
  if (!text) return false;
  return /\bnumber-flow-react\b/i.test(text)
    && /(\{[^}]*\}|--number-flow-|font-kerning|display\s*:\s*inline-block|calc\()/i.test(text);
}
