...
assert.match(runtime, /Get more add-ons/u);
assert.match(runtime, /https:\/\/bonklek\.github\.io\/milXdy\//u);
assert.match(runtime, /Load downloaded add-ons/u);
assert.match(runtime, /picker\.type = "file"/u);
assert.match(runtime, /picker\.multiple = true/u);
assert.match(runtime, /Queued add-ons/u);
