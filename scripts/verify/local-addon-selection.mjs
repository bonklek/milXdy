...
const composerTools = repositoryCatalog.sections[0].packages.find((pkg) => pkg.id === "composerTools");
const catalog = structuredClone(repositoryCatalog);
catalog.catalogId = "verify-maintainer-catalog";
catalog.revision = "verify-1";
catalog.sections[0].packages = [{ ...composerTools, availability: "published", blockers: [] }];
...
assert.throws(() => validateSelection({ ...selection, packages: [{ ...selection.packages[0], url: "https://example.com/pkg.zip" }] }, catalog, policy), /Unknown package composerTools field/u);
...
await writeFile(`${root}/reviews.json`, JSON.stringify({
  ...
  reviews: [{
    ...
    version: composerTools.version,
    packageSha256: composerTools.artifact.packageSha256,
    reviewedBy: composerTools.review.reviewedBy,
    reviewedAt: composerTools.review.reviewedAt,
  }],
}));
...
const stagedManifestPath = `${staging}/composerTools/milxdy.app.json`;
