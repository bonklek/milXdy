...
import { allowedMarkdownArchiveFiles, commonAssetDirs, contentScriptMatches, releaseBuilds, webAccessibleMatches } from "./release-builds.mjs";
...
import { publishFileAtomically, withReleaseArtifactLock, writeFileAtomically } from "./release-artifact-lock.mjs";
...
await withReleaseArtifactLock("package release archives", async () => {
  const checksums = [];
...
async function createZip(sourceDir, archive) {
  const tempArchive = `${archive}.tmp-${process.pid}-${Date.now()}-${Math.random().toString(36).slice(2)}`;
  try {
    await createDeterministicZip(sourceDir, tempArchive);
    await publishFileAtomically(tempArchive, archive);
  } finally {
...
async function verifyArchive(archive, build) {
  ...
  assertMarkdownArchivePolicy(zip, archive);
...
function assertArchiveManifestShape(archive, build, manifest) {
  ...
  if (build.target === "firefox") {
    ...
    assertFirefoxGeckoSettings(manifest, archive);
  } else {
...
function assertFirefoxGeckoSettings(manifest, label) {
  ...
  assertEqualList([...required].sort(), [
    ...
    "websiteActivity",
    "websiteContent",
  ].sort(), `${label}: Firefox data collection categories mismatch`);
}

function assertMarkdownArchivePolicy(zip, archive) {
  const actual = listZipEntries(zip)
    .map((entry) => entry.fileName)
    .filter((fileName) => fileName.toLowerCase().endsWith(".md"))
    .sort();
  assertEqualList(actual, [...allowedMarkdownArchiveFiles].sort(), `${archive}: Markdown archive payload allowlist mismatch`);
}
...
async function writeChecksumManifest(entries) {
  ...
  await writeFileAtomically(checksumFile, `${lines.join("\n")}\n`);
}
