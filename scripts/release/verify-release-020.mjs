...
const files = {
  ...
  extensionRuntime: await readFile("src/platform/background/extension-runtime.ts", "utf8"),
  updateCheck: await readFile("src/platform/background/update-check.ts", "utf8"),
  popup: await readFile("src/extension/popup/index.ts", "utf8"),
  overlayDock: await readFile("src/platform/overlay/dock.ts", "utf8"),
  remistatsBackground: await readFile("src/apps/remistats/background.js", "utf8"),
  reminetBackground: await readFile("src/apps/reminet-chat/background.ts", "utf8"),
  wikiContent: await readFile("src/apps/wiki-links/content.ts", "utf8"),
  wikiPreview: await readFile("src/apps/wiki-links/preview.ts", "utf8"),
  wikiSidebar: await readFile("src/apps/wiki-sidebar/content.ts", "utf8"),
  packageRelease: await readFile("scripts/release/package-release.mjs", "utf8"),
  appSmokeVerifier: await readFile("scripts/smoke/app-smoke-020.mjs", "utf8"),
  liveSmokeProbe: await readFile("scripts/smoke/live-smoke-probe-020.js", "utf8"),
  liveSmokeProbePrinter: await readFile("scripts/smoke/print-live-smoke-probe-020.mjs", "utf8"),
  liveSmokeProbeLogVerifier: await readFile("scripts/smoke/verify-live-probe-log-020.mjs", "utf8"),
  releaseGateVerifier: await readFile("scripts/release/verify-release-gates-020.mjs", "utf8"),
  build: await readFile("scripts/build/build-extension.mjs", "utf8"),
  buildProfiles: await readFile("scripts/build/build-profiles.mjs", "utf8"),
  releaseBuilds: await readFile("scripts/release/release-builds.mjs", "utf8"),
  releaseRegistry: await readFile("scripts/release/release-registry.mjs", "utf8"),
...
function verifyDocsCoverage() {
  ...
  assert(files.qaLog.includes("npm run print:live-probe:020") && files.qaLog.includes("scripts/smoke/live-smoke-probe-020.js"), "0.2.0 QA evidence log must document live probe tooling");
  ...
  assert(files.liveSmokeProbePrinter.includes("scripts/smoke/live-smoke-probe-020.js"), "live smoke probe printer must emit the shared probe file");
  ...
  assert(files.releaseGateVerifier.includes("node_modules/typescript/bin/tsc") && files.releaseGateVerifier.includes("scripts/build/build-profiles.mjs"), "0.2.0 gate runner must typecheck and rebuild profile outputs");
  ...
  for (const gate of [
    "scripts/release/verify-release-020.mjs",
    "scripts/verify/platform.mjs",
    "scripts/verify/url-allowlist.mjs",
    "scripts/verify/music-build.mjs",
    "scripts/smoke/extension-smoke-020.mjs",
    "scripts/smoke/app-smoke-020.mjs",
    "scripts/release/package-release.mjs",
    "scripts/release/verify-release-checksums.mjs",
    "scripts/release/verify-reproducible-release.mjs",
  ]) {
    assert(files.releaseGateVerifier.includes(gate), `0.2.0 gate runner missing ${gate}`);
  }
  assert(!files.releaseGateVerifier.includes("scripts/smoke/verify-live-probe-log-020.mjs"), "0.2.0 gate runner must keep live Chrome proof as a separate manual/browser gate");
  ...
  assert(files.packageRelease.includes("assertArchiveFeatureSet") && files.packageRelease.includes("src/platform/app-sdk/first-party-apps.json"), "release packaging must verify archived feature bundles against the app registry");
